...
def import_to_data(file_url, field_data, m2m_fields=None):
    ...
    读取导入的excel文件
    :param file_url:
    :param field_data: 首行数据�?    :param m2m_fields: 多对多字�?    :return:
    """
    # 读取excel 文件
    ...
    theader = tuple(table.values)[0] #Excel的表�?    is_update = '更新主键(勿改)' in theader #是否导入更新
    if is_update is False: #不是更新�?删除id�?        field_data.pop('id')
    # 获取参数映射
        ...
        for index, item in enumerate(field_data.items()):
            ...
            if cell_value is None or cell_value=='':
                ...
                try:
                    ...
                    raise CustomValidationError('日期格式不正�?)
