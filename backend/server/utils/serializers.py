...
class CustomModelSerializer(DynamicFieldsMixin, ModelSerializer):
    ...
    # 修改人的审计字段名称, 默认modifier, 继承使用时可自定义覆�?    modifier_field_id = "modifier"
    ...
    # 创建人的审计字段名称, 默认creator, 继承使用时可自定义覆�?    creator_field_id = "creator"
    creator_name = serializers.SlugRelatedField(
        slug_field="name", source="creator", read_only=True
    )
    # 数据所属部门字�?    dept_belong_id_field_name = "dept_belong_id"
    # 添加默认时间返回格式
