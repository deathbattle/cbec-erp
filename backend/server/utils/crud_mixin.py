...
class FastCrudMixin:
    ...
    定义快速CRUD数据操作的通用方法
    """
    # 需要CRUD的字�?    crud_fields = None
    # 排除CRUD的字�?    exclude_fields = None
    # 自定义CRUD的JSON
    ...
    # 获取字段属性信�?    def __get_field_attribute(self):
        result = []
        ...
        if self.crud_fields and type(self.crud_fields == list):
            for item in self.crud_fields:
                try:
                    ...
                    __name = field.name
                    # 判断类型是否为外键类�?外键类型需要特殊方式获取verbose_name
            ...
            # 获取model的所有字段及属�?            model_fields = queryset.model._meta.get_fields()
            # 遍历所有字段属�?            for field in model_fields:
                field_type = field.get_internal_type()
                __name = field.name
                # 判断需要排除的CRUD字段
