...
from server.system.models import ApiWhiteList, RoleMenuButtonPermission


def ValidationApi(reqApi, validApi):
    """
    验证当前用户是否有接口权�?    :param reqApi: 当前请求的接�?    :param validApi: 用于验证的接�?    :return: True或者False
    """
...
class CustomPermission(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if request.user.is_superuser:
            ...
            userApiList = RoleMenuButtonPermission.objects.filter(role__in=role_id_list).values(permission__api=F('menu_button__api'), permission__method=F('menu_button__method'))  # 获取当前用户的角色拥有的所有接�?            ApiList = [
                str(item.get('permission__api').replace('{id}', '([a-zA-Z0-9-]+)')) + ":" + str(
