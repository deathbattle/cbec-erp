...
from server.utils.json_response import ErrorResponse
...
def CustomExceptionHandler(ex, context):
    """
    统一异常拦截处理
    目的:(1)取消所有的500异常响应,统一响应为标准错误返�?        (2)准确显示错误信息
    :param ex:
    :param context:
    :return:
    """
    msg = ''
    code = 4000
    # 调用默认的异常处理函�?    response = exception_handler(ex, context)
    if isinstance(ex, AuthenticationFailed):
        # 如果是身份验证错�?        if response and response.data.get('detail') == "Given token not valid for any token type":
            code = 401
            msg = ex.detail
