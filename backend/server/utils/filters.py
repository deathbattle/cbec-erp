...
from server.system.models import Dept, ApiWhiteList, RoleMenuButtonPermission
from server.utils.models import CoreModel
...
def get_dept(dept_id: int, dept_all_list=None, dept_list=None):
    """
    递归获取部门的所有下级部�?    :param dept_id: 需要获取的部门id
    :param dept_all_list: 所有部门列�?    :param dept_list: 递归部门list
    :return:
    """
...
class DataLevelPermissionsFilter(BaseFilterBackend):
    """
    数据 级权限过滤器
    0. 获取用户的部门id，没有部门则返回�?    1. 判断过滤的数据是否有创建人所在部�?"creator" 字段,没有则返回全�?    2. 如果用户没有关联角色则返回本部门数据
    3. 根据角色的最大权限进行数据过�?会有多个角色，进行去重取最大权�?
    3.1 判断用户是否为超级管理员角色/如果�?(所有数�? 则返回所有数�?
    ...
    def _extracted_from_filter_queryset_33(self, request, queryset, api, method):
        # 0. 获取用户的部门id，没有部门则返回�?        user_dept_id = getattr(request.user, "dept_id", None)
        if not user_dept_id:
            return queryset.none()

        # 1. 判断过滤的数据是否有创建人所在部�?"dept_belong_id" 字段
        ...
        if _pk: # 判断是否是单例查�?            re_api = re.sub(_pk,'{id}', api)
        ...
        for ele in role_permission_list:
                # 判断用户是否为超级管理员角色/如果拥有[全部数据权限]则返回所有数�?            if ele.get("data_range") == 3:
                return queryset
            ...
            class AutoFilterSet(self.filterset_base):
                ...
                def get_filters(cls):
                    ...
                    for field_name, lookups in fields.items():
                        ...
                        # 不进�?过滤的model �?                        if isinstance(field, (models.JSONField, TimeZoneField)):
                            continue
