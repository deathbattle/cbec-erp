...
class CoreInitialize:
    """
    使用方法：继承此类，重写 run方法，在 run 中调�?save 进行数据初始�?    """
    creator_id = None
    reset = False
    request = request
    file_path = None
    ...
    def __init__(self, reset=False, creator_id=None, app=None):
        """
        reset: 是否重置初始化数�?        creator_id: 创建人id
        """
    ...
    def init_base(self, Serializer, unique_fields=None):
        ...
        with open(path_file,encoding="utf-8") as f:
            for data in json.load(f):
                filter_data = {}
                # 配置过滤条件,如果有唯一标识字段则使用唯一标识字段，否则使用全部字�?                if unique_fields:
                ...
                serializer.save()
        print(f"[{self.app}][{model._meta.model_name}]初始化完�?)
    ...
    def save(self, obj, data: list, name=None, no_reset=False):
        ...
        for ele in data:
            ...
            for key, value in ele.items():
                # 判断传的 value �?list 的多对多进行抽离，使用set 进行更新
