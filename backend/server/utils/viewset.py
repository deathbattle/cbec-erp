...
from server.utils.filters import DataLevelPermissionsFilter, CoreModelFilterBankend
from server.utils.import_export_mixin import ExportSerializerMixin, ImportSerializerMixin
from server.utils.json_response import SuccessResponse, ErrorResponse, DetailResponse
from server.utils.permission import CustomPermission
from server.utils.models import get_custom_app_models, CoreModel
from server.system.models import FieldPermission, MenuField
...
class CustomModelViewSet(ModelViewSet, ImportSerializerMixin, ExportSerializerMixin, QueryArgumentsMixin):
    """
    自定义的ModelViewSet:
    统一标准的返回格�?新增,查询,修改可使用不同序列化�?    (1)ORM性能优化, 尽可能使用values_queryset形式
    (2)xxx_serializer_class 某个方法下使用的序列化器(xxx=create|update|list|retrieve|destroy)
    (3)filter_fields = '__all__' 默认支持全部model中的字段查询(除json字段�?
    (4)import_field_dict={} 导入时的字段字典 {model�? model的label}
    ...
    # 通过many=True直接改造原有的API，使其可以批量创�?    def get_serializer(self, *args, **kwargs):
        serializer_class = self.get_serializer_class()
        kwargs.setdefault('context', self.get_serializer_context())
        # 全部以可见字段为�?        can_see = self.get_menu_field(serializer_class)
        # 排除掉序列化器级的字�?        # sub_set = set(serializer_class._declared_fields.keys()) - set(can_see)
        # for field in sub_set:
        #     serializer_class._declared_fields.pop(field)
        # if not self.request.user.is_superuser:
        #     serializer_class.Meta.fields = can_see
        # 在分页器中使�?        self.request.permission_fields = can_see
