...
from server.utils.import_export import import_to_data
from server.utils.json_response import DetailResponse
from server.utils.request_util import get_verbose_name
...
class ImportSerializerMixin:
    ...
    # 表格表头最大宽度，默认50个字�?    export_column_width = 50
    ...
    def import_data(self, request: Request, *args, **kwargs):
        ...
        if request.method == "GET":
            ...
            # 导出excel �?            response = HttpResponse(content_type="application/msexcel")
            ...
            # 添加数据�?            ws1.append(list(validation_data_dict.keys()))
            ...
            tab = Table(displayName="Table1", ref=f"A1:{row}{column}")  # 名称管理�?            style = TableStyleInfo(
            ...
            # 从excel中组织对应的数据结构，然后使用序列化器保�?            queryset = self.filter_queryset(self.get_queryset())
            # 获取多对多字�?            m2m_fields = [
                ele.name
    ...
    def update_template(self,request):
        ...
        for index, width in enumerate(df_len_max):
            ws.column_dimensions[get_column_letter(index + 1)].width = width
        tab = Table(displayName="Table", ref=f"A1:{row}{column}")  # 名称管理�?        style = TableStyleInfo(
            name="TableStyleLight11",
            showFirstColumn=True,
            showLastColumn=True,
            showRowStripes=True,
