...
from server.utils.json_response import DetailResponse, SuccessResponse, ErrorResponse
from server.utils.models import get_all_models_objects
...
class SystemConfigViewSet(CustomModelViewSet):
    ...
    def get_table_data(self, request, pk):
        ...
        if setting is None:
            return ErrorResponse(msg="查询出错了~")
        table = setting.get('table')  # 获取model�?        model = get_all_models_objects(table).get("object", {})
        # 自己判断一下不存在
...
class InitSettingsViewSet(APIView):
    ...
    def get(self, request):
        ...
        if not data:
            ...
            data = dispatch.get_system_config()
        # 不返回后端专用配�?        backend_config = [f"{ele.get('parent__key')}.{ele.get('key')}" for ele in
