...
from server.system.models import  Role, MenuField
from server.utils.models import get_custom_app_models
from server.utils.viewset import CustomModelViewSet
from server.utils.serializers import CustomModelSerializer
from server.utils.json_response import DetailResponse, ErrorResponse, SuccessResponse
...
class MenuFieldViewSet(CustomModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if MenuField.objects.filter(menu=payload.get('menu'),model=model.__name__, field_name=payload.get('field_name')).exists():
            return ErrorResponse(msg='�?s�?字段权限已有，不可重复创�? % payload.get('title'))
