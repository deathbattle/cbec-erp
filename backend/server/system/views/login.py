...
from server.system.models import Users
from server.utils.json_response import ErrorResponse, DetailResponse
from server.utils.request_util import save_login_log
from server.utils.serializers import CustomModelSerializer
from server.utils.validator import CustomValidationError
...
class LoginSerializer(TokenObtainPairSerializer):
    """
    登录的序列化�?
    重写djangorestframework-simplejwt的序列化�?
    """
    ...
    def validate(self, attrs):
        ...
        if dispatch.get_system_config_values("base.captcha_state"):
            if captcha is None:
                raise CustomValidationError("验证码不能为�?)
            ...
            if self.image_code and five_minute_ago > self.image_code.expiration:
                ...
                raise CustomValidationError("验证码过�?)
            else:
                if self.image_code and (
                    ...
                    self.image_code and self.image_code.delete()
                    raise CustomValidationError("图片验证码错�?)

        user = Users.objects.get(username=attrs['username'])
        if not user.is_active:
            raise CustomValidationError("账号已被锁定,联系管理员解�?)
        try:
...
class LoginView(TokenObtainPairView):
    ...
    #     # username可能携带的不止是用户名，可能还是用户的其它唯一标识 手机�?邮箱
...
class LoginTokenSerializer(TokenObtainPairSerializer):
    ...
    default_error_messages = {"no_active_account": _("账号/密码不正�?)}

    def validate(self, attrs):
        if not getattr(settings, "LOGIN_NO_CAPTCHA_AUTH", False):
            return {"code": 4000, "msg": "该接口暂未开�?", "data": None}
        data = super().validate(attrs)
        data["name"] = self.user.name
        data["userId"] = self.user.id
        return {"code": 2000, "msg": "请求成功", "data": data}
