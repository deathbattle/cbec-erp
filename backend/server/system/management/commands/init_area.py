...
from server.system.models import Area
...
def main():
    with open(os.path.join(BASE_DIR, 'server', 'system', 'util', 'pca-code.json'), 'r', encoding="utf-8") as load_f:
        ...
        if dispatch.is_tenants_mode():
            ...
            for tenant in get_tenant_model().objects.exclude(schema_name='public'):
                with tenant_context(tenant):
                    print(f"租户[{connection.tenant.schema_name}]初始化数据开�?..")
                    main()
