...
if [ -f "$ENV_FILE" ]; then
    ...
    sed -i "s|DATABASE_PASSWORD = 'TRADE-API-SERVER'|DATABASE_PASSWORD = '$MYSQL_PASSWORD'|g" ./backend/conf/env.py
    sed -i "s|REDIS_PASSWORD = 'TRADE-API-SERVER'|REDIS_PASSWORD = '$REDIS_PASSWORD'|g" ./backend/conf/env.py
    echo "初始化密码创建成功"
fi

docker-compose up -d
docker exec trade-api-server-django python manage.py makemigrations
docker exec trade-api-server-django python manage.py migrate
docker exec trade-api-server-django python manage.py init
echo "欢迎使用trade-api-server项目"
